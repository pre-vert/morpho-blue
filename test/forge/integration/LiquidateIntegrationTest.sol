...
contract LiquidateIntegrationTest is BaseTest {
    ...
    //     (uint256 returnSeized, uint256 returnRepaid) = morpho.liquidate(marketParams, BORROWER, amountSeized, 0,
    // hex"");
    ...
    //     uint256 borrowShares = morpho.borrowShares(id, BORROWER);
    //     uint256 liquidationIncentiveFactor = _liquidationIncentiveFactor(marketParams.lltv);
    //     uint256 maxSharesRepaid = (params.amountCollateral - 1).mulDivDown(params.priceCollateral,
    // ORACLE_PRICE_SCALE)
    //         .wDivDown(liquidationIncentiveFactor).toSharesDown(morpho.totalBorrowAssets(id),
    ...
    //     uint256 expectedSeized = sharesRepaid.toAssetsDown(morpho.totalBorrowAssets(id),
    // morpho.totalBorrowShares(id))
    ...
    //     (uint256 returnSeized, uint256 returnRepaid) = morpho.liquidate(marketParams, BORROWER, 0, sharesRepaid,
    // hex"");
    ...
    //     );
    //     assertEq(
    //         morpho.totalSupplyAssets(id), params.totalSupplyBeforeLiquidation - params.expectedBadDebt, "total
    // supply"
    //     );
    // }
